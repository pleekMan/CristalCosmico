...
void setup() {
  ...
  crystal.setPosition(new PVector (0, 0, -8000) );
...
void draw() {
  ...
  for (int i=0; i<lightRays.size(); i++) {
    ...
    if (atOutro) {
      lightRays.get(i).setColor(color(0));
    } else {
      lightRays.get(i).setColor(color(255));
    }
...
void keyPressed() {
  ...
  if (key == 'o' || key == 'O') {
    ...
    crystal.setPosition(new PVector (0, 0, -8000) );
    crystal.setVelocity(new PVector(0, 0, 5));
